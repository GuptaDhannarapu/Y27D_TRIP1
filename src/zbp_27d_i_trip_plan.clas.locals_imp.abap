...
CLASS lhc_Y27D_I_TRIP_PLAN DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS sendmail FOR MODIFY
      IMPORTING keys FOR ACTION plan_details~sendmail RESULT result.
...
*
                          ...
                          %action-SendMail = COND #( WHEN <fs_so_head>-status = '*'
                                                          THEN if_abap_behv=>fc-o-enabled
                                                          WHEN <fs_so_head>-status = 'X'
                                                          THEN if_abap_behv=>fc-o-enabled
                                                          ELSE if_abap_behv=>fc-o-disabled ) ).
  ...
  METHOD earlynumbering_create.
    ...
    DATA(lv_number) = lt_entities[ 1 ]-Batch_Id.
...
*    SELECT FROM y27d_t_plan_det FIELDS MAX( batchid ) INTO @DATA(lv_fin_vbeln).
    LOOP AT lt_entities INTO DATA(ls_entity).
      IF ls_entity-Batch_Id IS INITIAL.
        APPEND VALUE #( %cid = ls_entity-%cid
                        %is_draft = ls_entity-%is_draft
                        batch_id = lv_new_number + 1 ) TO mapped-plan_details.
      ELSE.
        APPEND VALUE #( %cid = ls_entity-%cid
                          %is_draft = ls_entity-%is_draft
                          batch_id = ls_entity-Batch_Id ) TO mapped-plan_details.
      ENDIF.
